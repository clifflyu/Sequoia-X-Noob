...
数据层优先使用 [baostock](http://baostock.com) 拉取历史及增量日 K 数据（后复权）；登录、限额或接口错误时自动降级到 AKShare。两个数据源都受独立的串行锁与节流保护。
策略计算所用的后复权 K 线全部来自本地 SQLite，不依赖实时抓取。

交易计划里的委托价需要**不复权**口径，该路径按 **腾讯 → 东方财富** 顺序取数且**不落库**。腾讯的后复权基准与 baostock 不同且无法换算，混入 `stock_daily` 会静默污染策略数据，因此腾讯源被限制为只能服务「不复权 + 不落库」的调用。

需要注意：降级到 AKShare 后请求会直接打到东方财富的公开接口，仍受其按 IP 限流（触发后表现为连接被直接关闭）。因此规模过大的降级同步会被主动跳过，阈值见 `DataEngine.AKSHARE_DEGRADED_MAX_TASKS`。
...
- **数据源**：Baostock（串行访问；每日安全上限 45,000 次）→ AKShare 自动降级
